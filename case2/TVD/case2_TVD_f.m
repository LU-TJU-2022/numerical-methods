...
for i=1:Nx+1
    ...
    c1(i,j)=exp(-(x1(i)-x0).^2/2/sigmax/sigmax-(y1(i)-y0).^2/2/sigmay/sigmay);
    end
end

c2=c1;
[X,Y]=meshgrid(1:Nx+1,1:Ny+1);%%%%meshgrid,注意矩阵行数对应y，列数对应X，比�?*4列，则y=1:3，x=1:4
...
for n=1:M
    ...
    for i=3:Nx-1
        ...
        %%-------计算f(Ci+1/2,R),f(Ci+1/2,L),F(i+1/2),可更换�?�?--------%%
